...
contract AntimatterEnglishAuctionNFT is Configurable, IERC721Receiver {
    ...
    function createErc721(
        ...
        uint tokenAmount0 = 1;
        _create(name, token0, token1, tokenId, tokenAmount0, amountMax1, amountMin1, amountMinIncr1, amountReserve1, duration, TypeErc721);
    }
    ...
    function createErc1155(
        ...
        _create(name, token0, token1, tokenId, tokenAmount0, amountMax1, amountMin1, amountMinIncr1, amountReserve1, duration, TypeErc1155);
    }
    ...
    function _creatorClaim(uint index) private {
        ...
        if (currentBidderP[index] != address(0) && amount1 >= reserveAmount1P[index]) {
            if (pool.token1 == address(0)) {
                ...
                if (txFee > 0) {
                    // keep transaction fee in this contract
